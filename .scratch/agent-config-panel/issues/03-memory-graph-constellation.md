...
- [ ] 新增依赖 `streamlit-agraph`（requirements.txt）
- [ ] `apps/corpchat/search/memory_graph.py`:
...
- [ ] `memory_graph` 表（session_id PK + nodes/edges JSON）+ load/save
- [ ] 图谱渲染: `st.components` + streamlit-agraph `Config`（节点大小=连接密度、颜色=类型）
- [ ] 联动 1: 高怀疑度(≥7) → 风险标签节点（诈骗/old_friend_reconnect 等）`highlighted`
- [ ] 联动 2: 数据源 → 图谱只含选定来源实体
- [ ] 联动 3: 点击节点 → `st.session_state.search_query = node` + 触发搜索
- [ ] 测试: 实体/边抽取（确定性元数据）、关键词 top-N、数据源过滤、怀疑度高亮、渲染接收正确 nodes/edges、点击回填、全量回归绿
