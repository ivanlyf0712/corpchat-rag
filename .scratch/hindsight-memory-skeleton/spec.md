...
- **Temporal is now delivered** as a separate feature: see `.scratch/hindsight-temporal/` (spec + issue 01). The agreed direction was followed — rule-first parser, enlarge limit + post-filter for combined queries, dedicated pure-temporal branch bypassing RRF.
- **Graph-parallel fusion path is now delivered** as an opt-in feature: see `.scratch/hindsight-graph-parallel/` (spec + issue 01). `search(graph_parallel=True)` adds a structural-neighbor path to RRF fusion; the append-only `_graph_expand` contract (ADR-0001) remains the default.
